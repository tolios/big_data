...
def query5_sql_to_csv():
    ...
    # steps: 1. For each year and artist, find the average number of streams and apply INNER JOIN with the mappings
    #           dataframe to get the artist id of each song
    #        2. For each year, find the artist_id with the MAX value, from the average value calculated before
    #        3. Apply the MAX trick in order to select the year and artist_id columns
    #        4. INNER JOIN with the artists dataframe to get the name of the artist and return the results
    ...
    result.show(5)
    result.coalesce(1).write.csv("hdfs://master:9000/outputs/sql_csv_results/result_5", mode="overwrite")
